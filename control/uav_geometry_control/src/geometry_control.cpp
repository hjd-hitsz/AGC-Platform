...
GeometryControl::GeometryControl(Eigen::Vector3d kp, Eigen::Vector3d kv, Eigen::Vector3d kr, double static_thrust_rate)
    : kp_(kp), kv_(kv), kr_(kr), static_thrust_rate_(static_thrust_rate)
{
}
                                            ...
                                            double yaw_des, double yaw_rate_des)
{
    ...
    auto thrust_rate = std::min(static_thrust_rate_ / g_ * a_norm_cmd, 1.0);
    ...
    control << a_norm_cmd, w_cmd(0), w_cmd(1), w_cmd(2);
    return control;
}
